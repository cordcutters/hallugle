...
func genWebPage(search Search) string {
	resp, err := model.GenerateContent(ctx, genai.Text(fmt.Sprintf(`You must only generate an HTML, Javascript and CSS webpage code with the following description and name that is encased in ", also make sure that the CSS theme matches the subject of the site, for example: use darker colors for darker subjects, truly go wild with the styling and colors, and make the css feel like it is matching the subject, when making <img>s, set the source to http://placekitten.com/{width}/{height}, you must make the website look more modern and include all sorts of interesting scripts, and styles related to the subject. You must not use the backtick character in the response. Only respond with the code. 
